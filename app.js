/* Quran Word Cards — alignments fixed per request
   Front  = left-aligned big Arabic + top-aligned derivations
   Back   = ONLY a single-column, top-down Ayah breakdown (word → meaning)
*/
...
const showError=m=>{els.errorBanner.textContent=m;els.errorBanner.classList.remove('hidden');setTimeout(()=>els.errorBanner.classList.add('hidden'),6e3)};
...
// FRONT: derivations list (top-aligned column)
function renderDerivations(list, container){
  container.innerHTML='';
  if(!Array.isArray(list)||!list.length){
    const d=document.createElement('div');d.className='deriv-item';
    d.innerHTML='<div class="meaning">— কোনো ডেরিভেশন নেই —</div>';container.appendChild(d);return;
  }
  list.forEach(dv=>{
...
// BACK: single column, top-down ayah breakdown (no repeated word/pills)
function renderAyahGrid(ayah, container){
  container.innerHTML='';
  const grid=document.createElement('div');
  grid.className='ayah-grid one-col'; // one column of rows
...
function renderCard(){
  ...
  // FRONT (big Arabic left-aligned, derivations start at top)
  ...
  // BACK (only grid; hide the repeated bits)
  els.backSurahNameBn.textContent=''; // no pills
...
async function loadManifest(){
  try{
    ...
    const man=await r.json(); if(!Array.isArray(man)) throw new Error('manifest.json অবশ্যই array হবে');
    state.manifest=man;
    els.surahSelect.innerHTML='<option value="" disabled selected>সুরা বাছাই করুন…</option>';
    man.forEach((m,i)=>{const o=document.createElement('option');o.value=String(i);o.textContent=m.display_bn||m.display||m.filename;els.surahSelect.appendChild(o);});
...
async function loadSurahByIndex(i){
  ...
  try{
    ...
    const arr=await r.json(); const s=Array.isArray(arr)?arr[0]:arr;
...
function handleQuickGo(){
  ...
  const [sr,ay,wd]=v.split(':'); const targetAy=ay; const targetW=wd||null;
  const flatIdx=state.words.findIndex(x=>{const [s,a,w]=(x.word.word_id||'').split(':'); return (!targetW? a===targetAy : (a===targetAy && w===targetW));});
  if(flatIdx>=0){const pos=state.order.indexOf(flatIdx);state.idx=pos>=0?pos:flatIdx;ensureFront();renderCard();}
}
